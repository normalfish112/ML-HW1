...
##Training
dim = x.shape[1] + 1 
w1 = np.zeros(shape = (dim, 1 ))
w2 = np.zeros(shape = (dim, 1 ))
...
for T in range(15000):
    if(T % 100 == 0 ):
        print("T=",T)
        print("Loss:",np.power(np.sum(np.power((np.power(x,2).dot(w2))+x.dot(w1) - y, 2 ))/ x.shape[0],0.5))
    #(np.power(x,2).dot(w2))  #x^2*w
    gradient_1 = (-2) * np.transpose(x).dot(y-x.dot(w1)-(np.power(x,2).dot(w2))) #(y-xw)**2 partial by w
    adagrad_sum += gradient_1 ** 2
    w1 = w1 - learning_rate * gradient_1 / (np.sqrt(adagrad_sum) + 0.0005)
    
    gradient_2 = (-2) * np.transpose(np.power(x,2)).dot(y-x.dot(w1)-(np.power(x,2).dot(w2))) #(y-xw)**2 partial by w
    adagrad_sum += gradient_2 ** 2
    w2 = w2 - learning_rate * gradient_2 / (np.sqrt(adagrad_sum) + 0.0005)
...
answer = test_x.dot(w1)+(np.power(test_x,2).dot(w2))
